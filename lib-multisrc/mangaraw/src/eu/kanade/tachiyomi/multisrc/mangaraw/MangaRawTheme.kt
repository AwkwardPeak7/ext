...
abstract class MangaRawTheme(
    ...
    override fun mangaDetailsParse(document: Document) =
        SManga.create().apply {
            ...
            thumbnail_url =
                root.selectFirst(Evaluator.Tag("img"))?.run {
                    absUrl("data-src").ifEmpty { absUrl("src") }
                }
            description =
                root.select(Evaluator.Tag("p")).lastOrNull { it.text().isNotEmpty() }
                    ?.run {
                        select(Evaluator.Tag("br")).prepend("\\n")
                        text().replace("\\n ", "\n")
                    }
            genre =
                root.select(Evaluator.AttributeWithValueContaining("rel", "tag"))
