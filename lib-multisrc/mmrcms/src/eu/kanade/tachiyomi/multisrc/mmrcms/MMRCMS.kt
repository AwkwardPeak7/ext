...
abstract class MMRCMS
    ...
    constructor(
        ...
        private val chapterString: String =
            when (lang) {
                "es" -> "Capítulo"
                "fr" -> "Chapitre"
                else -> "Chapter"
        ...
        override fun latestUpdatesParse(response: Response): MangasPage {
            ...
            val manga =
                document.select(latestUpdatesSelector()).mapNotNull {
            ...
            val hasNextPage =
                latestUpdatesNextPageSelector()?.let {
        ...
        protected fun parseSearchDirectory(page: Int): MangasPage {
            val manga =
                searchDirectory.subList((page - 1) * 24, min(page * 24, searchDirectory.size))
                    .map {
                        SManga.create().apply {
        ...
        protected val detailStatusOngoing =
            hashSetOf(
                "ongoing",
                "مستمرة",
                "en cours",
         ...
         */
        protected open fun cleanChapterName(
            mangaTitle: String,
            name: String,
        ): String {
        ...
        protected open fun fetchFilterOptions() {
            ...
            scope.launch {
                try {
                    if (supportsAdvancedSearch) {
                        val document =
                            client.newCall(GET("$baseUrl/advanced-search", headers))
                                .await()
                                .asJsoup()

                        categories =
                            document.select("select[name='categories[]'] option").map {
                        ...
                        tags =
                            document.select("select[name='types[]'] option").map {
                                it.text() to it.attr("value")
                            }
                    } else {
                        val document =
                            client.newCall(GET("$baseUrl/$itemPath-list", headers))
                                .await()
                                .asJsoup()

                        categories =
                            document.select("a.category").map {
                                it.text() to it.attr("href").toHttpUrl().queryParameter("cat")!!
                            }
                        tags =
                            document.select("div.tag-links a").map {
                                it.text() to it.attr("href").toHttpUrl().pathSegments.last()
                            }
                        sortOptions =
                            document.select("#sort-types label:has(input)").map {
