...
class ZeistMangaIntl(lang: String) {
    ...
    val typeManhua: String =
        when (availableLang) {
            SPANISH -> "Manhua"
            else -> "Manhua"
        }

    val typeManhwa: String =
        when (availableLang) {
            SPANISH -> "Manhwa"
            else -> "Manhwa"
        }

    val typeNovel: String =
        when (availableLang) {
            SPANISH -> "Novela"
            else -> "Novel"
        }

    val typeWebNovelJP: String =
        when (availableLang) {
            SPANISH -> "Web Novel (JP)"
            else -> "Web Novel (JP)"
        }

    val typeWebNovelKR: String =
        when (availableLang) {
            SPANISH -> "Web Novel (KR)"
            else -> "Web Novel (KR)"
        }

    val typeWebNovelCN: String =
        when (availableLang) {
            SPANISH -> "Web Novel (CN)"
            else -> "Web Novel (CN)"
        }

    val typeDoujinshi: String =
        when (availableLang) {
            SPANISH -> "Doujinshi"
            else -> "Doujinshi"
        }
    ...
    val languageFilterTitle: String =
        when (availableLang) {
            SPANISH -> "Idioma"
            ARAB -> "لغة"
            INDONESIAN -> "Bahasa"
