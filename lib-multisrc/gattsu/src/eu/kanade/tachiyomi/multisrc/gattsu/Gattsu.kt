...
abstract class Gattsu(
    ...
    override fun mangaDetailsParse(document: Document): SManga =
        SManga.create().apply {
            ...
            genre =
                postBox.select("ul.post-itens li:contains(Tags) a")
                    .joinToString(", ") { it.text() }
            description =
                postBox.select("div.post-texto p")
                    .joinToString("\n\n") { it.text() }
                    .replace("Sinopse :", "")
                    .trim()
            status = SManga.COMPLETED
            thumbnail_url =
                postBox.select("div.post-capa > img.wp-post-image")
                    .attr("src")
                    .withoutSize()
        }
    ...
    override fun chapterFromElement(element: Element): SChapter =
        SChapter.create().apply {
            ...
            date_upload =
                element.ownerDocument()!!.select("meta[property=article:published_time]").firstOrNull()
                    ?.attr("content")
                    .orEmpty()
                    .toDate()
