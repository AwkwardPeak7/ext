...
class ColaMangaIntl(private val lang: String) {
    val rateLimitPrefTitle =
        when (lang) {
            "zh" -> "主站连接限制"
            else -> "Rate limit"
        }

    fun rateLimitPrefSummary(defaultValue: String) =
        when (lang) {
    ...
    val rateLimitPeriodPrefTitle =
        when (lang) {
            "zh" -> "主站连接限制期"
            else -> "Rate limit period"
        }

    fun rateLimitPeriodPrefSummary(defaultValue: String) =
        when (lang) {
    ...
    val timedOutDecryptingImageLinks =
        when (lang) {
            else -> "Timed out decrypting image links"
        }

    val couldNotDeobufscateScript =
        when (lang) {
            else -> "Could not deobfuscate script"
        }

    fun couldNotFindKey(forKeyType: String) =
        when (lang) {
