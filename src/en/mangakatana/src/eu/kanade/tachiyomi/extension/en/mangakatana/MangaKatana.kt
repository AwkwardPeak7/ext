...
class MangaKatana : ConfigurableSource, ParsedHttpSource() {
    ...
    override val client: OkHttpClient =
        network.cloudflareClient.newBuilder().addNetworkInterceptor { chain ->
    ...
    override fun pageListParse(document: Document): List<Page> {
        val imageScript =
            document.select("script:containsData(data-src)").firstOrNull()?.data()
                ?: return emptyList()
        val imageArrayName =
            imageArrayNameRegex.find(imageScript)?.groupValues?.get(1)
                ?: return emptyList()
