...
class LxHentai : ParsedHttpSource() {
            ...
            baseUrl.toHttpUrl().newBuilder().apply {
                ...
                (if (filters.isEmpty()) getFilterList() else filters).forEach {
                    when (it) {
                        ...
                        is Author ->
                            if (canAddTextFilter && it.state.isNotEmpty()) {
    ...
    override fun mangaDetailsParse(document: Document) =
        SManga.create().apply {
            ...
            author =
                document.select("div.grow div.mt-2:contains(Tác giả) span a")
                    .joinToString { it.text().trim(',', ' ') }
            genre =
                document.select("div.grow div.mt-2:contains(Thể loại) span a")
                    .joinToString { it.text().trim(',', ' ') }
            ...
            description += "\n"
            description +=
                document.select("p:contains(Tóm tắt) ~ p").joinToString("\n") {
                    it.run {
