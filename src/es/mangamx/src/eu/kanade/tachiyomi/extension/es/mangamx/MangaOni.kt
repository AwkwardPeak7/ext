...
open class MangaOni : ConfigurableSource, ParsedHttpSource() {
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        ...
        val mangas =
            if (document.location().startsWith("$baseUrl/directorio")) {
    ...
    override fun mangaDetailsParse(document: Document): SManga {
        ...
        manga.author =
            document.select("div#info-i").text().let {
        ...
        manga.status =
            when (document.select("strong:contains(Estado) + span").first()?.text()) {
    ...
    override fun pageListParse(document: Document): List<Page> {
        val encoded =
            document.select("script:containsData(unicap)").firstOrNull()
