...
open class NovelCool(
    ...
    override fun fetchPopularManga(page: Int): Observable<MangasPage> {
        return when (preference.useAppApi) {
            true ->
                client.newCall(commonApiRequest("$apiUrl/elite/hot/", page))
                    .asObservableSuccess()
    ...
    override fun fetchLatestUpdates(page: Int): Observable<MangasPage> {
        return when (preference.useAppApi) {
            true ->
                client.newCall(commonApiRequest("$apiUrl/elite/latest/", page))
                    .asObservableSuccess()
                    .map(::commonApiResponseParse)
    ...
    override fun fetchSearchManga(
        ...
        return when (preference.useAppApi) {
            true ->
                client.newCall(commonApiRequest("$apiUrl/book/search/", page, query))
                    .asObservableSuccess()
                    .map(::commonApiResponseParse)
    ...
    override fun pageListParse(document: Document): List<Page> {
        ...
        val images =
            imgRegex.find(script)?.groupValues?.get(1)
    ...
    private fun jsRedirect(chain: Interceptor.Chain): Response {
        ...
        val document = Jsoup.parse(response.peekBody(Long.MAX_VALUE).string())
        val jsRedirect =
            document.selectFirst("script:containsData(window.location.href)")?.html()
                ?.substringAfter("\"")
                ?.substringBefore("\"")
                ?: return response
        ...
        val url =
            "${requestUrl.scheme}://${requestUrl.host}$jsRedirect".toHttpUrlOrNull()
                ?: return response
    ...
    companion object {
        ...
        // copied from Madara
        private val completedStatusList: Array<String> =
            arrayOf(
                "completed",
                "completo",
                "completado",
        ...
        private val ongoingStatusList: Array<String> =
            arrayOf(
                "ongoing",
                "Продолжается",
                "updating",
