...
open class MangaToon(
    ...
    override fun searchMangaRequest(
        ...
        val searchUrl =
            "$baseUrl/$urlLang/search".toHttpUrl().newBuilder()
    ...
    override fun mangaDetailsParse(document: Document): SManga =
        SManga.create().apply {
            author =
                document.select("div.detail-author-name span").text()
                    .substringAfter(": ")
            description =
                document.select("div.detail-description-short p")
                    .joinToString("\n\n") { it.text() }
            genre =
                document.select("div.detail-tags-info span").text()
                    .split("/")
    ...
    override fun chapterListParse(response: Response): List<SChapter> {
        ...
        val firstPaid =
            PAID_CHECK_BREAKPOINTS.find { breakpoint ->
